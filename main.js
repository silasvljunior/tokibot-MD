   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Por favor insira seu numero de whatsapp : +5544333333333")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Por favor insira seu numero de whatsapp 😍\nPor exemplo: +5544333333333 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')

         // Ask again when entering the wrong number
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Por favor insira seu numero de whatsapp, Por exemplo: +5544333333333")))
...
try{
		...
		if (update.connection == "open" || update.receivedPendingNotifications == "true") {
            ...
            console.log(color(`🌿Conectado à => ` + JSON.stringify(XeonBotInc.user, null, 2), 'yellow'))
			await delay(1999)
            console.log(chalk.yellow(`\n\n               ${chalk.bold.blue(`[ ${botname} ]`)}\n\n`))
            console.log(color(`< ================================================== >`, 'cyan'))
	        console.log(color(`\n${themeemoji} CANAL DO YT: silasvljunior`,'magenta'))
            console.log(color(`${themeemoji} GITHUB: silasvljunior `,'magenta'))
            console.log(color(`${themeemoji} INSTAGRAM: @silasvljunior `,'magenta'))
...
// Anti Call
    ...
    let XeonBlokMsg = await XeonBotInc.sendTextWithMentions(XeonFucks.from, `*${XeonBotInc.user.name}* não recebo ${XeonFucks.isVideo ? `video` : `voice` } ligação. desculpe @${XeonFucks.from.split('@')[0]} você vai ser bloqueado. se ligou acidentalmente contate meu dono para desbloquear você @silasvljunior !`)
...
const xeondate = moment.tz('America/Sao_Paulo').format('DD/MM/YYYY')
let xeonName = num
xeonbody = ` Parabéns🎉 @${xeonName.split("@")[0]}, você foi promovido à *admin* 🥳`
   XeonBotInc.sendMessage(anu.id,
 { text: xeonbody,
 contextInfo:{
 mentionedJid:[num],
...
const xeondate = moment.tz('America/Sao_Paulo').format('DD/MM/YYYY')
let xeonName = num
xeonbody = `Oops‼️ @${xeonName.split("@")[0]}, você foi *removido* de *admin* 😬`
XeonBotInc.sendMessage(anu.id,
 { text: xeonbody,
 contextInfo:{
 mentionedJid:[num],
...
ppgroup = 'https://i.ibb.co/RBx5SQC/avatar-group-large-v2.png?q=60'
			...
			if (res.announce == true) {
				...
				XeonBotInc.sendMessage(res.id, {
					text: `「 Configurações do grupo mudadas 」\n\nO grupo foi fechado por *admins*, agora somente admins mandam mensagem !`,
				})
			} else if (res.announce == false) {
				await sleep(2000)
				XeonBotInc.sendMessage(res.id, {
					text: `「 Configurações do grupo mudadas 」\n\nO grupo foi aberto pelos *admins* !`,
				})
			} else if (res.restrict == true) {
				await sleep(2000)
				XeonBotInc.sendMessage(res.id, {
					text: `「 Configurações do grupo mudadas 」\n\nAs informações do grupo foram restringido, agora só *admins* podem editar !`,
				})
			} else if (res.restrict == false) {
				await sleep(2000)
				XeonBotInc.sendMessage(res.id, {
					text: `「 Configurações do grupo mudadas 」\n\nAs informações do grupo foram aberta, agora todos podem editar as informações !`,
				})
			} else if(!res.desc == ''){
				await sleep(2000)
				XeonBotInc.sendMessage(res.id, { 
					text: `「 Configurações do grupo mudadas 」\n\n*A descrição do grupo foi mudado por *\n\n${res.desc}`,
				})
      } else {
				await sleep(2000)
				XeonBotInc.sendMessage(res.id, {
					text: `「 Configurações do grupo mudadas 」\n\n*O nome do Grupo foi mudado por *\n\n*${res.subject}*`,
				})
			} 
			}
		})
